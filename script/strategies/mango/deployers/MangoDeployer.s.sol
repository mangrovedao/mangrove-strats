...
import {Mango, IERC20, IMangrove} from "mgv_strat_src/strategies/offer_maker/market_making/mango/Mango.sol";
 ...
 *
 *   WRITE_DEPLOY=true \
 *   BASE=WETH QUOTE=USDC TICK_SPACING=1 BASE_0=$(cast ff 18 1) QUOTE_0=$(cast ff 6 800)\
...
contract MangoDeployer is Deployer {
  ...
  function innerRun(
    ...
    string memory mangoName = getName(name, base, quote, tickSpacing);
    fork.set(mangoName, address(current));
  }

  function getName(string memory name, IERC20 base, IERC20 quote, uint tickSpacing) public view returns (string memory) {
    if (bytes(name).length > 0) {
      return name;
    } else {
      return string.concat("Mango_", base.symbol(), "_", quote.symbol(), "_", vm.toString(tickSpacing));
    }
  }
}
