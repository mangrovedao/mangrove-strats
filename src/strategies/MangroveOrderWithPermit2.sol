...
contract MangroveOrderWithPermit2 is BaseMangroveOrder {
  ...
  ///@notice pull inbound_tkn from the msg.sender with permit approval valid for `permit.expiration` time and then forward market order to MGV.
  ///@param outbound_tkn outbound_tkn.
  ///@param inbound_tkn inbound_tkn.
  ///@param takerWants Amount of outbound_tkn taker wants.
  ///@param takerGives Amount of inbound_tkn taker gives.
  ...
  ///@return totalPenalty Penalty received.
  ///@return feePaid Fee paid.
  function marketOrderWithPermit(
    IERC20 outbound_tkn,
    IERC20 inbound_tkn,
    uint takerWants,
    uint takerGives,
    ...
    uint pulled = Permit2Router(address(router())).pull(inbound_tkn, msg.sender, takerGives, true);
    require(pulled == takerGives, "mgvOrder/transferInFail");

    return marketOrderInternal(outbound_tkn, inbound_tkn, takerWants, takerGives, fillWants);
  }

  ///@notice pull inbound_tkn from the msg.sender assuming that permit approval is already setted up and then forward market order to MGV.
  ///@param outbound_tkn outbound_tkn.
  ///@param inbound_tkn inbound_tkn.
  ///@param takerWants Amount of outbound_tkn taker wants.
  ///@param takerGives Amount of inbound_tkn taker gives.
  ...
  ///@return totalPenalty Penalty received.
  ///@return feePaid Fee paid.
  function marketOrder(IERC20 outbound_tkn, IERC20 inbound_tkn, uint takerWants, uint takerGives, bool fillWants)
    external
    returns (uint totalGot, uint totalGave, uint totalPenalty, uint feePaid)
  {
    uint pulled = Permit2Router(address(router())).pull(inbound_tkn, msg.sender, takerGives, true);
    require(pulled == takerGives, "mgvOrder/transferInFail");
