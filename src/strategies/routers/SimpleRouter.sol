...
contract SimpleRouter is AbstractRouter {
  ...
  function __pull__(RL.RoutingOrder memory routingOrder, bool strict) internal virtual override returns (uint pulled) {
    ...
    uint amount = strict ? routingOrder.amount : routingOrder.token.balanceOf(routingOrder.fundOwner);
    if (TransferLib.transferTokenFrom(routingOrder.token, routingOrder.fundOwner, msg.sender, amount)) {
      return amount;
    } else {
      return 0;
    }
  ...
  function __push__(RL.RoutingOrder memory routingOrder) internal virtual override returns (uint) {
    bool success =
      TransferLib.transferTokenFrom(routingOrder.token, msg.sender, routingOrder.fundOwner, routingOrder.amount);
