...
abstract contract AbstractRouter is AbstractRoutingLogic {
  ...
  function pull(IERC20 token, uint amount, bytes calldata data) external override onlyBound returns (uint pulled) {
  ...
  function push(IERC20 token, uint amount, bytes calldata data) external override onlyBound returns (uint pushed) {
    if (amount == 0) {
      return 0;
    }
  ...
  ///@param data is a bytes array that can be used to pass arbitrary data to the router.
  function flush(IERC20[] calldata tokens, bytes calldata data) external override onlyBound {
  ...
  function checkList(IERC20 token, bytes calldata data) external override view {
  ...
  ///@param data is a bytes array that can be used to pass arbitrary data to the router.
  function activate(IERC20 token, bytes calldata data) external override boundOrAdmin {
  ...
  ///@param token the asset one wishes to use the router for
  ///@param data is a bytes array that can be used to pass arbitrary data to the router.
  function __activate__(IERC20 token, bytes calldata data) internal override virtual {
    token; //ssh
    data;
  }
}
